...
if [[ "$CURRENT_BRANCH" == session/* ]]; then
  # Confirm git can actually operate here before trusting its output. Piping
  # status into `wc -l` forces exit status 0, so a hard git failure (e.g.
  # "dubious ownership") yields CHANGED=0 — indistinguishable from a clean
  # tree — and the whole commit/push block is skipped without a word.
